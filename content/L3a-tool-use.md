...
- **Complexity:** 2 of 5. The mechanism is one extra round-trip: the model emits a structured call, you run it, you hand the result back. The cost is in the boundary judgment (which tools are safe to expose) and in validating the model's arguments, not in the loop itself.
...
- *structured call* — the model's request to use a tool, formatted as machine-readable fields (tool name + arguments), not free text.
...
Recipe: query test cards, tap-to-reveal. This section makes the read-only vs side-effectful boundary vivid. The verdict here is not "good question / bad question." It is **L3a (read-only, belongs on this page)** versus **L5 Action (side-effectful, belongs elsewhere)**, plus a couple of cases where no tool is needed at all.

The framing: a tool is L3a when it only reads. The instant it writes, sends, books, or clicks, it crosses into L5 Action. The work can be identical. The boundary is the side effect.
...
2. **"Book me the 9am flight to Cairo."**
   ...
   - Reason: Booking writes a reservation and charges a card, so the side effect moves it to L5 Action.
...
5. **"What's 17.5% of 2,480,000?"**
   - Verdict: Read-only tool ✓
   - Reason: A calculator tool computes and returns a number with no side effect, so it is L3a (and beats letting the model do mental arithmetic).

6. **"Summarize this paragraph for me."**
   - Verdict: No tool needed ✗
   - Reason: The answer lives in the model's own language ability, so calling a tool adds a round-trip for nothing.
...
Recipe: case-file dossier, one specific real read-only case. Use a weather lookup end to end.

**User asks:** "Do I need an umbrella in Cairo today?"

**Step 1 — Model decides a tool is needed**
The model has no live weather in its training data, so it picks the `get_weather` tool instead of guessing.

**Step 2 — Model emits a structured call**
It produces a structured call, not prose:
`get_weather(city="Cairo", units="metric")`
This is the request. The model does not run it.

**Step 3 — Your code runs the tool**
Your code executes the function and calls the weather API. The API only reads current conditions, so nothing in the world changes. It returns:
`{ "condition": "clear", "temp_c": 31, "precip_mm": 0 }`

**Step 4 — Result is fed back to the model**
You hand the JSON result back to the model in a follow-up message. Now the model has the fact it was missing.

**Step 5 — Model answers from the result**
The model reads the result and writes a plain-language answer grounded in it.

**Answer returned to user:**
> "No umbrella needed. Cairo is clear and 31°C today, with no rain forecast."

The whole exchange was read-only: the model only fetched data. Nothing was booked, written, or sent.
...
- Lines 2–3: Declare the tool's schema: its name and the arguments it takes. This is all the model knows about it.
...
- **Complexity — Medium (2–3/5):** The loop is simple. The real work is validating the model's arguments and keeping exposed tools strictly read-only.
