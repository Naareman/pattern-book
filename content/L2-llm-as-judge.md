...
- **self-preference bias:** A judge's tendency to rate text written by its own model family more highly.

---

## 03 · WHEN IT WORKS · WHEN IT FAILS

Clean boundary here: judging works when "good" is well-defined and checkable from the answer plus rubric. It fails when the criterion is subjective, requires ground truth the judge lacks, or triggers a known bias. Six tap-to-reveal cards (3 positive, 3 negative).

**Card 1 — Handles it ✓**
"Does this support reply stay polite and on-policy?"
Tone and policy adherence are visible in the text and easy to score against a rubric.

**Card 2 — Handles it ✓**
"Which of these two draft summaries is more faithful to the source?"
Pairwise comparison with the source attached is where judges align best with humans, if you swap the order to cancel position bias.
...
The judge has no reliable ground truth; it may confidently confirm a wrong but fluent answer.
...
- **self-preference bias:** A judge's tendency to rate text from its own model family more highly.
